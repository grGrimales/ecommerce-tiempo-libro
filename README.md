...
- **ItemDetail:** Su función es crear el cart con la descripcion del producto selecionado en ver Detalles. Recibe por props el productId, contiene la función handleOnAdd la cual añade el producto al carrito con la función addItem recibe por parametro el producto e isInCart ecibe por parametro el Id y la cantidad de producto hace la verificación si ya existe el producto en el carrito le suma la nueva cantidad seleccionada, una vez añadido muestra un botón para finalizar la compra con un Link que te lleva a cart. Este componente es llamado en ItemDetailContainer.
